    ...
    async isFieldRequired(fieldName) {
        ...
        if (typeof window !== 'undefined' && window.appBridge) {
            try {
                ...
                const syncManager = window.appBridge.apps.get('mapping_slayer')?.syncAdapter?.syncManager;
                if (syncManager && syncManager.getSignsWithFieldData) {
                    const signsWithData = await syncManager.getSignsWithFieldData(this.code, fieldName);
...
export class SignInstance {
    ...
    toDot() {
        const dot = {
            ...
            notes: this.notes,
            isCodeRequired: this.codeRequirements ? true : false,
            installed: this.installed
        };
    ...
    static fromDot(dot, pageNumber = 1) {
        ...
        Object.keys(dot).forEach(key => {
            ...
            else if (!['internalId', 'markerType', 'x', 'y', 'locationNumber', 
                      'notes', 'isCodeRequired', 'installed'].includes(key)) {
        ...
        return new SignInstance({
            ...
            messages: messages,
            notes: dot.notes || '',
            codeRequirements: dot.isCodeRequired ? 'Required' : '',
